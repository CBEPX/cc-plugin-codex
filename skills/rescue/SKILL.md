...
Resolve `<plugin-root>` as two directories above this `SKILL.md` file. Keep the shell tool in the active Codex user workspace; never set its working directory to `<plugin-root>` or the directory used to read this skill. Pass that shell's absolute current workspace explicitly as `--cwd "$PWD"`:
`node "<plugin-root>/scripts/claude-companion.mjs" task --cwd "$PWD" ...`
...
- If none of `--resume`, `--resume-last`, or `--fresh` is present, first run:
  `node "<plugin-root>/scripts/claude-companion.mjs" task-resume-candidate --cwd "$PWD" --json`
...
- Before spawning the built-in child, capture the task job id plus routing context in one call:
  `node "<plugin-root>/scripts/claude-companion.mjs" background-routing-context --kind task --cwd "$PWD" --json`
