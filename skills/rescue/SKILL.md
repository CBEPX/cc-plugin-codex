---
name: rescue
description: 'Delegate a substantial diagnosis, implementation, or follow-up task to Claude Code through the tracked-job runtime. Args: --background, --wait, --guest <claude|grok>, --resume, --resume-last, --fresh, --write, --model <model|opus|sonnet|haiku|fable>, --effort <low|medium|high|xhigh|max>, --wait-timeout-ms <ms>, --prompt-file <path>, [task text]. Defaults to opus + xhigh effort. Default guest is claude. Use when Claude should investigate or change things, not when the user only wants review findings.'
---
...
Supported arguments: `--background`, `--wait`, `--guest <claude|grok>`, `--resume`, `--resume-last`, `--fresh`, `--write`, `--model <model|opus|sonnet|haiku|fable>`, `--effort <low|medium|high|xhigh|max>`, `--wait-timeout-ms <ms>`, deprecated alias `--timeout-ms <ms>`, `--prompt-file <path>`, plus free-text task text (default guest is claude)
...
- Treat `--guest`, `--model`, `--effort`, `--wait-timeout-ms`, deprecated alias `--timeout-ms`, `--resume`, `--resume-last`, `--fresh`, and `--prompt-file` as runtime or routing controls, not task text.
- Preserve explicit `--guest <claude|grok>` exactly. Do not add `--guest grok` unless the user asked for Grok.
