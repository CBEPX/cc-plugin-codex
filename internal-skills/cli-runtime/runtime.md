...
The public rescue skill already resolved the active plugin root from its `SKILL.md` path and the canonical user workspace from `background-routing-context`. Reuse both. Never derive the workspace from the plugin root or the directory used to read this document.
