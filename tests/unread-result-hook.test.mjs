...
test("bounds expired queued-job transitions by the prompt deadline", (t) => {
  if (process.platform !== "darwin") return t.skip("Darwin ps fixture");
  const testEnv = createEnv();
  try {
    initGitRepo(testEnv.workspaceDir);
    writeJob(testEnv, {
      id: "stale-queued-no-pid", sessionId: "session-a", status: "queued",
      pid: null, workerPid: null,
      createdAt: "2026-01-01T00:00:00Z", updatedAt: "2026-01-01T00:00:00Z",
    });
    const bin = path.join(testEnv.rootDir, "slow-transition-ps");
    fs.mkdirSync(bin);
    const executable = path.join(bin, "ps");
    fs.writeFileSync(executable, "#!/usr/bin/env node\nsetInterval(() => {}, 1000);\n", "utf8");
    fs.chmodSync(executable, 0o755);
    ...
    assert.ok(performance.now() - startedAt < 2_200);
    assert.equal(output, "");
    assert.equal(readJob(testEnv, "stale-queued-no-pid").status, "queued");
  } finally {
    cleanupEnv(testEnv);
  }
});
