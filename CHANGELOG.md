...
- Clarify the optional review gate as a turn-end blocking review of the previous Codex turn, keep gate runs on the user's Claude Code default model/effort settings, surface detected Claude defaults in setup, and stamp gate snapshots with an explicit `turn-end` trigger.
